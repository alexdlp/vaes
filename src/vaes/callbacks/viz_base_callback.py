...
class VizBaseCallback(Callback, ABC):
    ...
    def _get_mlflow_path(self, filename: str) -> str:
        ...
        artifact_uri_local = artifact_uri.as_posix().replace('file:', '')
    ...
    def on_fit_start(self, pipeline: "BasePipeline") -> None:
        ...
        logger.info(f"[{self.__class__.__name__}] Model attached.")
    ...
    def on_epoch_end(self, pipeline: "BasePipeline", epoch: int, logs: Dict[str, float]) -> None:
        ...
        if epoch == 1 or epoch % self.plot_every_n_epochs == 0:
            logger.info(
                f"[{self.__class__.__name__}] Generating validation plots for epoch {epoch}..."
            )

            samples = self.collect_samples(pipeline=pipeline)
            if not samples:
                logger.warning(
                    f"{self.__class__.__name__}: no samples collected for epoch {epoch}. Skipping plot."
                )
                return
            ...
            logger.info(f"[{self.__class__.__name__}] Saved visualization: {pdf_path}")
    ...
    def on_fit_end(self, pipeline: "BasePipeline") -> None:
        ...
        if already_plotted_last_epoch:
            logger.info(
                f"[{self.__class__.__name__}] Training completed after {total_epochs} epochs. "
